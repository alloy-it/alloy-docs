...
Browse [alloy-it.io](https://alloy-it.io) in your browser, or list available blueprints from the command line:

```bash
alloy-provisioner clone --list community
```

This prints all community blueprints and their available tags. Look for one that matches your board or toolchain.
...
cp -r alloy-catalog/blueprints/nordic/nrf91 ./my-blueprint
```
...
git commit -m "feat: extend nrf91 blueprint with project tools"
```
