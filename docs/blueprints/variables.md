...
Use a `.env` file for values that change per machine or contain credentials: registry tokens, private artifact URLs, or variable overrides. These should **never be committed**.
...
# DEV_USERNAME is auto-detected from the sudo context (see below).
# Override here only if provisioning for a different user.
# DEV_USERNAME=another-user
```
...
```bash
sudo alloy-provisioner install --blueprint-dir /path/to/blueprint -env-file /path/to/.env
```
...
## 4. DEV_USERNAME auto-detection

Blueprints that run tasks as a non-root user (via `run_as:`) use a `DEV_USERNAME` variable to identify that user. The provisioner resolves it automatically. **You do not need to set it** for standard use.

Resolution order:

1. `DEV_USERNAME` already set in the environment or `.env` → used as-is, no override.
2. `$SUDO_USER` is set (i.e. you ran `sudo alloy-provisioner …`) → used automatically.
3. `$USER` is set and is not `root` → used as fallback.

The provisioner logs which source was used:

```text
DEV_USERNAME auto-detected from $SUDO_USER: "ubuntu"
```

**When you need to override it:** provisioning for a different user than the one running `sudo`, or running in a CI environment where `SUDO_USER` and `USER` are both `root`. In that case set it explicitly:

```bash
sudo alloy-provisioner install --blueprint-dir /path/to/blueprint -env-file /path/to/.env
# .env contains: DEV_USERNAME=ci-build-user
```
...
Set them in a .env file and run:
  set -a && source .env && set +a && alloy-provisioner install --blueprint-dir .
```
