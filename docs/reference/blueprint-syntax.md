...
For standalone `alloy-provisioner` usage, these tags are activated directly by
provisioner flags/environment variables and do not depend on `alloy-host`.

When running inside [alloy-host](../installing/with-alloy-host.md), environment
tags are activated by the selected backend (for example WSL2 activates `wsl2`,
Docker activates `docker`).
...
Run the task as a specific user instead of root. Use `{{.Vars.DEV_USERNAME}}` rather than a hardcoded username; the provisioner auto-detects the correct value from the sudo context at runtime:
