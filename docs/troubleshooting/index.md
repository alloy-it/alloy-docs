...
2. **Custom registry:** If you use a private registry, set `--registry` or `ALLOY_REGISTRY`:
   ...
   ```bash
   sudo alloy-provisioner install myorg/my-blueprint:1.0.0 --registry https://api.alloy-it.io
   ```
...
- Check the blueprint name and tag (e.g. `community/nordic/nrf91:1.1.3`, `community/raspberry-pi-arm64:1.0.0`). Browse [Alloy Hub](https://alloy-it.io) to confirm the exact name and available tags.
...
- Re-running the provisioner is safe: it skips completed tasks (tracked in `alloy.state.yml`). To force a full re-run, remove the state file and run again:
  ...
  sudo alloy-provisioner install --blueprint-dir /path/to/blueprint
  ```
- For blueprint from registry: `sudo alloy-provisioner install community/nordic/nrf91:1.1.3` again will re-use the pulled blueprint and only re-run tasks that need to run.
