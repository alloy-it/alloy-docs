...
1. **From Alloy Hub (registry)** using `install`.
2. **From a local blueprint directory** using `--blueprint-dir`.

Registry example (Alloy Hub):

```dockerfile
ARG BLUEPRINT_REF=community/nordic/nrf91:1.1.3
...
When provisioning inside a Docker build, pass `--docker` so blueprint tasks with
`exclude: [docker]` are skipped.

This is important for tasks that are not valid in typical containers (for
example: reloading udev rules, privileged kernel operations, or hardware-specific
setup).
...
Use either approach:
- Prefer `--docker` for explicitness in command examples.
- Use `ALLOY_ENV_DOCKER=1` when you want tag activation shared across multiple
  `RUN` steps.
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build firmware
        run: |
          cmake -GNinja -DCMAKE_TOOLCHAIN_FILE=/opt/nordic/cmake/nrf9160.cmake -B build
          ninja -C build
```
...
If your Docker build fails on host-integrated tasks (for example `udevadm`
errors), check:

1. The task has `exclude: [docker]` in your blueprint.
2. The Docker invocation activates the docker tag (`--docker` or
   `ALLOY_ENV_DOCKER=1`).
3. The same blueprint still runs without `--docker` on native Linux/VM when that
   task should execute there.
