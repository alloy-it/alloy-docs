...
Blueprints that install SDKs or tools as a non-root user rely on a `DEV_USERNAME` variable. You do not need to set this manually. When you run `sudo alloy-provisioner`, the provisioner automatically detects the calling user from `$SUDO_USER` (or falls back to `$USER`) and uses it. No `.env` entry is needed.
